...
function loadState() {
  try {
    ...
    if (saved.settings && typeof saved.settings === 'object') {
      ...
      if (typeof s.autoAdvance    === 'boolean') state.settings.autoAdvance     = s.autoAdvance && tierConfig.autoAdvance;
...
function setTier(tier) {
  ...
  state.tier = tier;
  const maxItems = APP_CONFIG.tiers[tier].maxSetlistItems;
  if (state.setlist.length > maxItems) {
    state.setlist = state.setlist.slice(0, maxItems);
    if (state.currentSetlistIndex >= state.setlist.length) {
      state.currentSetlistIndex = state.setlist.length - 1;
    }
    renderSetlist();
